...
passport.serializeUser(function(user, done) {
  done(null, {isPartner: user.isPartner , id : user.id});
});
...
passport.deserializeUser(function(human, done) {
  if (human.isPartner) {
    Partner.findById(human.id, function(err, user) {
      console.log(human.id);
      done(err, user);
    })
  } else {
    User.findById(human.id, function(err, user) {
      done(err, user);
    });
  }
});
...
passport.use("partner",
  new LocalStrategy(
    ...
    function(userName, password, done) {
      Partner.findOne({ userName: userName }, function(err, user) {
        if (err) return done(err);
